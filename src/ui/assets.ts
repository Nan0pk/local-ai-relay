    ...
    for (const provider of providers) {
      ...
      if (provider.status === "disabled") {
        ...
        if (provider.latest_job?.status === "running") {
          ...
          actions.append(button(provider.ready ? "Verify again" : "Connect and sign in", "button primary", () => providerAction(provider, "connect").catch(showError)));
        }
